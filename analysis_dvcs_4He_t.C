...
void analysis_dvcs_4He_t::Loop() 
{
  ...
  // 20 days at 1.5e34
      ...
      for(int i=0; i<n_con; i++)
      {
         ...
         hh_Q2[i] = new TH1D(Form("hh_Q2[%d]",i),"", 150, 0, 30.0);

         h_Q2_tt_Coh[i]  = new TH2D(Form("h_Q2_tt_Coh[%d]",i),"",150, 0.9, 50, 150, 0.0, 0.1);
         h_Q2_xB_Coh[i]  = new TH2D(Form("h_Q2_xB_Coh[%d]",i),"",150, 0.0, 0.1, 150, 0.9, 30);
         ...
         for(int j=0; j<n_xB; j++)
         {
            for(int k=0; k<n_t; k++)
            {
               ...
               h_t_Q2_Coh[i][j][k] = new TH1D(Form("h_t_Q2_Coh[%d][%d][%d]",i,j,k),"",150, 0.5, 30);
