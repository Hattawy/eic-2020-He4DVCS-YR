...
const int Npx = 100;
 ...
 CANfits->Print("figs/png/clas12_Him_slopes.png");
 CANfits->Print("figs/pdf/clas12_Him_slopes.pdf");


 // plot the profile densities ------------------------------------
 TCanvas *CANprofs = new TCanvas("CANprofs","CANprofs",650,500);
    //      CANprofs->Divide(7,1);
 ...
 for(int ifit=0;ifit<n_xB;ifit++){
    ...
    TF1 *fun_prof = new TF1(Form("prof_%d",ifit+1),"[0]*TMath::Exp(-x*x/(4*[1]))",0,0.35);
         ...
         if (ifit ==0 ) fun_prof->Draw();
         else fun_prof->Draw("same");
 ...
 CANprofs->Print("figs/png/clas12_profiles.png");
 CANprofs->Print("figs/pdf/clas12_profiles.pdf");
 ...
 TGaxis *zaxis = new TGaxis(X0,Z0,X1,Z1,0,0.2,605);
 ...
 Z0 = (0.-obs_z)*(X[0]-obs_y)/(X[0]-obs_y);
 X1 = (1.-obs_x)*(X[0]-obs_y)/(X[8]-obs_y);
 Z1 = (0.-obs_z)*(X[0]-obs_y)/(X[8]-obs_y);
  TF1 *funaxis = new TF1("funaxis",Form("(0.-%f)*(%f-%f)/(x-%f)",obs_x,X[0],obs_y,obs_y),X[0],X[8]);
 ...
 TLine *vert_ax;
        ...
        Z0 = (0.-obs_z)*(X[0]-obs_y)/(X[0]-obs_y);
        X1 = (0.-obs_x)*(X[0]-obs_y)/(X[8]-obs_y);
        Z1 = (1.-obs_z)*(X[0]-obs_y)/(X[8]-obs_y);
                ...
                for(int idiv=1;idiv<5;idiv++)
                 {
                    ...
                    Z0 = (0.-obs_z)*(X[0]-obs_y)/(X[0]-obs_y);
                    X1 = (0.2*idiv-obs_x)*(X[0]-obs_y)/(X[0]-obs_y);
 ...
 for(int ifit=0;ifit<n_xB;ifit++)
   {
     Float_t persp_fact;
     if(ifit ==0 || ifit==8)  persp_fact = (X[ifit]-obs_y)/(X[8]-obs_y);
     else   persp_fact = (X[0]-obs_y)/(X[ifit]-obs_y);
 ...
 // draw b and z axis again
  X0 = (0.-obs_x)*(X[0]-obs_y)/(X[8]-obs_y);
  Z0 = (0.-obs_z)*(X[0]-obs_y)/(X[8]-obs_y);
  X1 = (1.-obs_x)*(X[0]-obs_y)/(X[8]-obs_y);
  Z1 = (0.-obs_z)*(X[0]-obs_y)/(X[8]-obs_y);
 ...
 TLatex *comments = new TLatex();
         ...
         //icomments->DrawLatex(-1.8,-1.96,"^{4}He quark density profiles");

   TLegend* leg = new TLegend(0.72,0.62,0.97,0.80);
   leg-> SetNColumns(2);
   leg->SetTextSize(0.02);
   //leg->AddEntry(profile_fun2d[0]," Impulse Appr. Model","L");
   //leg->AddEntry(profile_G2D[0]," Projected uncertainties","f");
   for( int i=0; i<n_xB; i++){
   leg->AddEntry(profile_G2D[i],Form("<x_{B}>= %0.4f",X[i]),"f");
   }
   leg->Draw("same");


c0->Print("figs/png/nucl_profile.png");
c0->Print("figs/pdf/nucl_profile.pdf");
