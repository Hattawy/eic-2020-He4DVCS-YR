...
const int Npx = 100;
 ...
 for(int ifit=0;ifit<3;ifit++)
   ...
   for(int point=0;point<Npx;point++)
    ...
    else if (ifit ==2) {profile_G2D[ifit]->SetFillColor(40); profile_G2D[ifit]->SetLineColor(40);   }
 ...
 TLatex *comments = new TLatex();
   ...
   leg->AddEntry(profile_G2D[0],Form("<x_{B}>= %0.2f",X[0]),"f");
   leg->AddEntry(profile_G2D[1],Form("<x_{B}>= %0.2f",X[1]),"f");
   leg->AddEntry(profile_G2D[2],Form("<x_{B}>= %0.2f",X[2]),"f");
   leg->Draw("same");
